...
class CORE_API UPackageMap : public UObject
{
	...
	virtual FClassNetCache* GetClassNetCache( UClass* Class ); //Generates a cache for a new class
	virtual UBOOL SupportsPackage( UObject* InOuter ); //Needed by GetClassNetCache
...
protected:
	TMap<UObject*,INT> LinkerMap; //These objects are actually ULinkerLoad's
