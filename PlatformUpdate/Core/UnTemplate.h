...
public:
	...
	FORCEINLINE INT AddZeroed( INT n=1 )
	{
		guardSlow(TArray::AddZeroed);
		return FArray::AddZeroed( sizeof(T), n );
		unguardSlow;
	}
	FORCEINLINE INT AddUniqueItem( const T& Item )
	{
	...
	FORCEINLINE INT RemoveItem( const T& Item )
	{
...
public:
	...
	void ModifyAllItems()
	{
		...
		if( GUndo )
			GUndo->SaveArray( Owner, this, 0, this->ArrayNum, 0, sizeof(T), SerializeItem, DestructItem );
		unguardSlow;
	}
...
public:
    ...
    T& operator()( INT i )
	{
		...
		checkSlow(ArrayMax>=this->ArrayNum);
		if( SavedPos>0 )
			Load();
