...
// A game level.
//
class ENGINE_API ULevelBase : public UObject, public FNetworkNotify //Size=140
{
...
class ENGINE_API ULevel : public ULevelBase
{
	...
	TArray<FReachSpec>		ReachSpecs;	//O=140
	UModel*					Model;	//O=152
	UTextBuffer*			TextBlocks[NUM_LEVEL_TEXT_BLOCKS]; //O=156
	FTime                   TimeSeconds; //O=220
	TMap<FString,FString>	TravelInfo; //O=228

	// Only valid in memory.
	FCollisionHashBase* Hash; //O=248
	class FMovingBrushTrackerBase* BrushTracker; //O=252
	AActor* FirstDeleted; //O=256
	struct FActorLink* NewlySpawned; //O=260
	UBOOL InTick, Ticked; //O=264,268
	INT iFirstDynamicActor, iFirstNetRelevantActor, NetTag; //O=272,276,280
	BYTE ZoneDist[64][64]; //284 (size=4096)

	// Temporary stats. //4380, 4384, 4388			, 4392				, 4396			, 4400		, 4404	, 4408	, 4412	, 4416				, 4420	, 4424	, 4428		, 4432
	...
	virtual void WelcomePlayer( UNetConnection* Connection, TCHAR* Optional=(TCHAR*)TEXT("") );
